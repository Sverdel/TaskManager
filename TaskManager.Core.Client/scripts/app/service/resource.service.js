System.register(["@angular/core", "./../service/auth.http", "./../service/auth.service"], function (exports_1, context_1) {
    "use strict";
    var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
        var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
        if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
        else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
        return c > 3 && r && Object.defineProperty(target, key, r), r;
    };
    var __metadata = (this && this.__metadata) || function (k, v) {
        if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
    };
    var __moduleName = context_1 && context_1.id;
    var core_1, auth_http_1, auth_service_1, ResourceService;
    return {
        setters: [
            function (core_1_1) {
                core_1 = core_1_1;
            },
            function (auth_http_1_1) {
                auth_http_1 = auth_http_1_1;
            },
            function (auth_service_1_1) {
                auth_service_1 = auth_service_1_1;
            }
        ],
        execute: function () {
            ResourceService = class ResourceService {
                constructor(http, authService) {
                    this.http = http;
                    this.authService = authService;
                    this.states = null;
                    this.priorities = null;
                    this.baseUrl = "http://localhost:8000/api/resources/";
                    this.init();
                }
                init() {
                    if (!this.authService.isLoggedIn()) {
                        return;
                    }
                    this.getStates()
                        .subscribe(states => {
                        this.states = states;
                    });
                    this.getPriorities()
                        .subscribe(priorities => {
                        this.priorities = priorities;
                    });
                }
                isInitialized() {
                    return this.states != null && this.priorities != null;
                }
                getStates() {
                    return this.http.get("http://localhost:8000/api/resources/states")
                        .map(response => {
                        return response.json();
                    });
                }
                getPriorities() {
                    return this.http.get("http://localhost:8000/api/resources/priorities")
                        .map(response => {
                        return response.json();
                    });
                }
            };
            ResourceService = __decorate([
                core_1.Injectable(),
                __metadata("design:paramtypes", [auth_http_1.AuthHttp, auth_service_1.AuthService])
            ], ResourceService);
            exports_1("ResourceService", ResourceService);
        }
    };
});
